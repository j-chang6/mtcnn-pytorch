...
class MTCNN(object):
    ...
    def generate_bbox(self, cls_map, reg, scale, threshold):
        ...
        """
        ##这里的stride是pnet会对原图进行压缩的比例，进行反采样
        stride = 2
        cellsize = 12
        face_map = cls_map[1, :, :]
        ##t_index是(x,y)网格点的坐标，特征图上的位置，他在原图上表示的是一个区域
...
if __name__ == "__main__":
    ...
    # rnet
    rnet_weight_path = "scripts/models/rnet_20181218_final.pkl"
    rnet = RNet(test=True)
    LoadWeights(rnet_weight_path, rnet)
    rnet.to(device)

    # onet
    onet_weight_path = "scripts/models/onet_20181218_final.pkl"
    onet = ONet(test=True)
    ...
    img_path = r"D:\python_code\mtcnn-pytorch-master\img\wo3.jpg"
