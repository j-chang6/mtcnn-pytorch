...
class MTCNN(object):
    ...
    def __init__(self, detectors=[None,None,None], min_face_size=12, scalor=0.79, threshold=[0.6,0.7,0.8],
...
def LoadWeights(path, net):
    ...
    ##这里的weights是一个字典，k为features.0.weight，v是一个tensor
...
if __name__ == "__main__":
    ...
    # pnet
    # pnet_weight_path = "scripts/models/pnet_20181218_final.pkl"
    pnet_weight_path = "scripts/weights/pnet_20200327_final.pkl"
    pnet = PNet(test=True)
    LoadWeights(pnet_weight_path, pnet)
    pnet.to(device)

      # rnet
    rnet_weight_path = "scripts/weights/rnet_20200327_final.pkl"
    rnet = RNet(test=True)
    LoadWeights(rnet_weight_path, rnet)
    rnet.to(device)

    # onet
    onet_weight_path = "scripts/weights/onet_20200327_final.pkl"
    onet = ONet(test=True)
    LoadWeights(onet_weight_path, onet)
    onet.to(device)

    mtcnn = MTCNN(detectors=[pnet,rnet,onet], device=device, threshold=[0.6,0.7,0.8])


    # img_path = r"D:\python_code\mtcnn-pytorch-master\img\wo2.png"
    img_path = r"D:\python_code\mtcnn-pytorch-master\img\poeple3.jpg"
